...
// -- Cart Logic --
let cart = JSON.parse(localStorage.getItem('wilsonCart') || '{}');
function saveCart() { localStorage.setItem('wilsonCart', JSON.stringify(cart)); }
...
productsList.addEventListener('click', e => {
  if (e.target.tagName === 'BUTTON') {
    ...
    setTimeout(() => {e.target.textContent = "Add to Cart";}, 850);
  }
});
...
function renderCart() {
  ...
  if (total === 0) cartItemsDiv.innerHTML = '<em>Your cart is empty.</em>';
...
document.getElementById('contact-form').onsubmit = function(e){
  e.preventDefault();
  alert('Thanks for contacting Wilson! We will reply soon.');
  this.reset();
};
